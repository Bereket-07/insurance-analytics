...
def train_each_model_using_gridsearchcv(lr_grid,dt_grid,rf_grid,xgb_grid,X_train_scaled,y_train_claims):
    lr_grid.fit(X_train_scaled, y_train_claims)
    dt_grid.fit(X_train_scaled, y_train_claims)
    rf_grid.fit(X_train_scaled, y_train_claims)
    xgb_grid.fit(X_train_scaled, y_train_claims)
    return lr_grid,dt_grid,rf_grid,xgb_grid
def train_each_model(lr_model, dt_model, rfr_model, xgb_model, X_train_scaled, y_train_claims):
    ...
    lr_model.fit(X_train_scaled, y_train_claims)
    print("finfish lr")
    dt_model.fit(X_train_scaled, y_train_claims)
    print("finfish dt")
    rfr_model.fit(X_train_scaled, y_train_claims)
    print("finfish rfr")
    xgb_model.fit(X_train_scaled, y_train_claims)
    print("finfish xgb")
    return lr_model, dt_model, rfr_model, xgb_model
